...
function Utility.EnemiesNearLocation(loc,dist)
	...
	for _,enID in pairs(GetTeamPlayers(GetTeam())) do
		...
		if IsHeroAlive(enID) and Utility.GetDistance(enemyInfo['location'],loc)<=dist and (Utility.GetDistance(enemyInfo['location'],Vector(0,0))>10) and enemyInfo['time']<30 then
...
function Utility.UseItems()
	...
	local midas=Utility.IsItemAvailable("item_hand_of_midas");
	if midas~=nil and midas:IsFullyCastable() then
		creeps=npcBot:GetNearbyLaneCreeps(600,true);
		if creeps~=nill and #creeps>0 and creeps[1]~=nil then
			npcBot:Action_UseAbilityOnEntity(midas,creeps[1]);
		end
	end
